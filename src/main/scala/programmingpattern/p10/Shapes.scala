...
object Main extends App {
  ...
  // 面积
  val areas = for {
    shape <- SecondShapeExample.someShapes
  } yield shape.area
  val areas2 = for (shape <- SecondShapeExample.someShapes) yield shape.area
  println(areas)
  println(areas2)

  // 加入的正方形
  val areas3 = for (shape <- ThirdShapeExample.someMoreShapes) yield shape.area
  println(areas3)
}
